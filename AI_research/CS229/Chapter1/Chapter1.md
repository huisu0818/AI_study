...
$$
\begin{align*}
\hat{\theta}(x_1, \ldots, x_m) &= \argmax_{\theta} L(\theta)\\
&=\argmax_{\theta}\ell(\theta)\\
&=\argmax_{\theta}-\frac{1}{2}\sum_{i=1}^{m}\left(y^i-\theta^{T}x^i\right)^2\\
&=\argmin_{\theta}\frac{1}{2}\sum_{i=1}^{m}\left(y^i-\theta^{T}x^i\right)^2\\
&=\argmin_{\theta} J(\theta)
\end{align*}
$$
